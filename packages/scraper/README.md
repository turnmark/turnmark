...
```php
// 例: ボートレースの公式サイトから 2026年05月31日 の開催場を取得
$stadiums = Scraper::scrapeStadium('2026-05-31');
...
- **払戻金の `label`** : 組番が確定しなかった賭式は `combination` が `null` になり、ページに印字された文言（`特払` / `不成立` など）が `label` にそのまま入ります。**文言の種類は列挙していない**ため、解釈は利用側で行ってください。組番が確定した通常の賭式では `label` は `null` です。
- **払戻金の `amount`** : 金額が読めなかった行は、行ごと捨てずに `amount` を `null` にして残します。組番や文言は取れているのに金額だけ取れないのは表の状態ではなくページ構成の変化が疑われるため、痕跡を残す方針です。組番も文言も無い空欄行は従来どおり返しません。
- **`refunds` / `remarks`** : 返還艇と備考です。返還が無いレースでも表自体は存在するため、`refunds` は空配列、`remarks` は `null` になります。
- **直前情報の `parts`** : `null` は直前情報そのものが未公開、空配列は「公開されているが部品交換なし」を表します。`quantity` は個数が印字されている部品にのみ入り、印字が無い部品は `1` ではなく `null` です。
- **オッズ** : セルが数値でない場合（欠場など）は `0.0` ではなく `null` を返します。取得できなかった場合も `null` です。
